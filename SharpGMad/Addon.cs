...
namespace SharpGMad
{
    ...
    public class Addon
    {
        ...
        public Dictionary<string, List<ContentFile>> DictionaryFiles = new Dictionary<string, List<ContentFile>>();
        ...
        public Dictionary<string, List<ContentFile>> GetDictionaryFiles()
        {
            ...
            foreach (ContentFile item in this.Files)
            {
                //keyValuePairs.Add(item.Path, item);
                if (Path.GetDirectoryName(item.Path) is string folder)
                {
                    if (DictionaryFiles.ContainsKey(folder))
                    {
                        DictionaryFiles[folder].Add(item);
                    }
                    else
                    {
                        DictionaryFiles.Add(folder, new List<ContentFile>() { item });
                    }
                }
            }
            return DictionaryFiles;
    ...
    public class ContentFile
    {
        ...
        {
            return $"[{(double)this.Size / 1024} mb] {this.Path}";
        }
        public string ToStringType(Type type)
        {
            double d = this.Size / 1024.0;

            var vvv = Convert.ChangeType(d, type);
